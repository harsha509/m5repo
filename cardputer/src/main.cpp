...
static void startBt() {
  ...
  bleInit(btName);
  wifiInit(WIFI_SSID, WIFI_PASS, WIFI_TOKEN);
}
...
void drawInfo() {
  ...
  if (infoPage == INFO_PG_SESSIONS) {
    ...
    for (uint8_t i = 0; i < tama.nLines && y < H - 10; i++) {
      bool busy = strncmp(tama.lines[i], "[busy]", 6) == 0;
      spr.setTextColor(busy ? p.text : p.textDim, p.bg);
      spr.setCursor(4, y); spr.printf("%.39s", tama.lines[i]); y += 10;
    }
