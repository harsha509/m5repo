...
enum class HalKey : uint8_t {
  ...
  VolUp,         // =
  VolDown,       // -
  Reason,        // r — type a denial reason
  Sessions,      // s — jump to the sessions window
  Usage,         // u — jump to the usage window
  Other,         // o — type a free-text answer to a question
};
HalKey halPollKey();

// Speaker volume, 0..100. Stateless: the persisted level lives in Settings.
void halVolume(int pct);

// Text entry. While enabled, printable keys queue as characters instead of
// firing shortcuts; Enter and Del still arrive via halPollKey() as
// Approve / Back so callers can commit or cancel. Returns 0 when empty.
