...
struct Settings {
  ...
  uint8_t bright;    // 0..4, mirrors brightLevel so it survives reboot
  uint8_t volume;    // 0..100 percent
};

static Settings _settings = { true, true, false, true, true, 0, 4, 63 };
...
inline void settingsLoad() {
  ...
  _settings.bright = _prefs.getUChar("s_bri", 4);
  if (_settings.bright > 4) _settings.bright = 4;
  _settings.volume = _prefs.getUChar("s_vol", 63);
