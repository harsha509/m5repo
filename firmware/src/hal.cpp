...
// StickC has no keyboard or volume control; keep the symbols for a common main.cpp.
void halVolume(int)        {}
void halSetTextMode(bool)  {}
char halPollChar()         { return 0; }
...
// M5.Display.setBrightness expects 0..255, pct comes in as 0..100. wake() in
// main.cpp re-applies the stored level right after halScreenPower(true).
...
void  halVolume(int pct) { M5.Speaker.setVolume((pct * 255) / 100); }
...
char     _charQ[KEY_Q_SIZE];
uint8_t  _charHead = 0, _charTail = 0;

void _pushChar(char c) {
  uint8_t next = (_charHead + 1) % KEY_Q_SIZE;
  if (next == _charTail) return;
  _charQ[_charHead] = c;
  _charHead = next;
}
...
void _pollKeyEvents() {
  auto& st = M5Cardputer.Keyboard.keysState();
  // Text entry takes the printable keys verbatim; shortcuts stay dormant so
  // typing "no" can't deny the very prompt you are annotating.
  if (_textMode) {
    if (!st.fn) {
      for (char c : st.word) {
        if (_wordContains(_prevWord, c)) continue;
        if (c >= 32 && c < 127) _pushChar(c);
      }
    }
  ...
  if (!st.fn) {
    for (char c : st.word) {
      ...
      switch (c) {
        ...
        case ']':           k = HalKey::BrightUp;   break;
        case '[':           k = HalKey::BrightDown; break;
        case '=':           k = HalKey::VolUp;      break;
        case '-':           k = HalKey::VolDown;    break;
        case 'r': case 'R': k = HalKey::Reason;     break;
        case 's': case 'S': k = HalKey::Sessions;   break;
        case 'u': case 'U': k = HalKey::Usage;      break;
        case 'o': case 'O': k = HalKey::Other;      break;
      }
...
void halSetTextMode(bool on) {
  _textMode = on;
  _charHead = _charTail = 0;   // drop anything typed before the mode switch
}

char halPollChar() {
  if (_charHead == _charTail) return 0;
  char c = _charQ[_charTail];
  _charTail = (_charTail + 1) % KEY_Q_SIZE;
  return c;
}
