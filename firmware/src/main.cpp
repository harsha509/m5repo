...
bool spriteReady = false;   // false means the UI is drawing into nothing
...
static void startBt() {
  ...
  bleInit(btName);
  wifiInit(WIFI_SSID, WIFI_PASS, BROKER_HOST, BROKER_PORT, BROKER_TOKEN);
  otaInit(BROKER_HOST, BROKER_PORT, BROKER_TOKEN);
}
...
// True while the Sessions window owns Up/Down/Enter/Back. Enter is wired to both
// halBtnA() and HalKey::Approve on Cardputer, and BtnA is handled first, so the
// button path must defer here or the key path never sees the page it needs.
static inline bool onSessionsPage();

// Sessions window: which row is highlighted, and the action sheet over it.
uint8_t sessSel = 0;
bool    sessActionsOpen = false;
uint8_t sessActionSel = 0;
const char* SESS_ACTIONS[] = { "stop", "respawn", "delete", "logs", "say: run tests", "back" };
const uint8_t SESS_ACTION_N = 6;
uint8_t petPage = 0;
...
uint8_t  promptScroll = 0;   // row offset into the wrapped command view
char     denyReason[72] = "";
bool     denyMode = false;   // text entry open (deny reason, or a typed answer)
bool     askText = false;    // text entry is answering a question, not denying
uint8_t  askSel = 0;         // highlighted option on a question prompt

// Emits an answer for the live question, JSON-escaped via ArduinoJson so an
// option label containing quotes cannot break the frame.
static void sendCmd(const char* json);
static void sendAnswer(const char* text) {
  JsonDocument d;
  d["cmd"] = "answer"; d["id"] = tama.promptId; d["a"] = text;
...
const uint8_t INFO_PG_SESSIONS = 6;
const uint8_t INFO_PG_USAGE = 7;

static inline bool onSessionsPage() {
  return displayMode == DISP_INFO && infoPage == INFO_PG_SESSIONS;
}
...
const char* settingsItems[] = { "brightness", "sound", "bluetooth", "wifi", "led", "transcript", "clock rot", "ascii pet", "update", "reset", "back" };
...
// Blocking OTA check behind a status screen. Menu-only: OTA cannot yet finish a
// 1.9MB transfer over WiFi (dies partway, ~250KB in), so it must not run at boot.
static void otaRun(bool fromMenu) {
  const Palette& p = characterPalette();
  spr.fillSprite(p.bg);
  spr.setTextDatum(MC_DATUM);
  spr.setTextSize(1);
  spr.setTextColor(p.text, p.bg);
  spr.drawString("checking for update...", W / 2, H / 2);
  spr.pushSprite(0, 0);

  OtaResult result = otaCheck(fromMenu);
  if (fromMenu || result != OTA_UP_TO_DATE) {
    spr.fillSprite(p.bg);
    spr.setTextColor(p.textDim, p.bg);
    spr.drawString(otaStatus(), W / 2, H / 2);
    spr.pushSprite(0, 0);
    delay(1200);
  }
...
// Sends one control command for the highlighted session. Only a session id and
...
static void sessControl(uint8_t action) {
  ...
  char cmd[128];
  switch (action) {
    case 0: snprintf(cmd, sizeof(cmd), "{\"cmd\":\"stop\",\"id\":\"%s\"}", id); break;
    case 1: snprintf(cmd, sizeof(cmd), "{\"cmd\":\"respawn\",\"id\":\"%s\"}", id); break;
    case 2: snprintf(cmd, sizeof(cmd), "{\"cmd\":\"rm\",\"id\":\"%s\"}", id); break;
    case 3: snprintf(cmd, sizeof(cmd), "{\"cmd\":\"logs\",\"id\":\"%s\"}", id); break;
    case 4: snprintf(cmd, sizeof(cmd),
              "{\"cmd\":\"say\",\"id\":\"%s\",\"macro\":\"run_tests\"}", id); break;
    default: return;
  }
  wifiControl(cmd);
}
...
static void applySetting(uint8_t idx) {
  ...
  switch (idx) {
    case 0:
      ...
      applyBrightness();
      s.bright = brightLevel;   // fall through to settingsSave()
      break;
...
void drawInfo() {
  ...
  if (infoPage == INFO_PG_SESSIONS) {
    ...
    spr.setTextColor(p.textDim, p.bg);
    ln("%u total  %u busy  %u blocked", tama.sessionsTotal, tama.sessionsRunning, tama.sessionsWaiting);
    y += 2;
    if (tama.nLines == 0) {
      spr.setTextColor(p.text, p.bg);
      ln("no sessions");
    }
    if (sessActionsOpen && sessSel < tama.nLines) {
      spr.setTextColor(p.text, p.bg);
      spr.setCursor(4, y); spr.printf("%.39s", tama.lines[sessSel]); y += 12;
      for (uint8_t i = 0; i < SESS_ACTION_N && y < H - 10; i++) {
        bool sel = (i == sessActionSel);
        spr.setTextColor(sel ? p.text : p.textDim, p.bg);
        spr.setCursor(8, y);
        spr.print(sel ? "> " : "  ");
        spr.print(SESS_ACTIONS[i]);
        y += 10;
      }
    } else {
      ...
      if (wifiControlResult()[0]) {
        spr.setTextColor(p.body, p.bg);
        spr.setCursor(4, H - 10); spr.printf("%.38s", wifiControlResult());
      }
    }
  } else if (infoPage == INFO_PG_USAGE) {
    _infoHeader(p, y, "USAGE", infoPage);
    spr.setTextColor(p.text, p.bg);
    ln("today   %lu tok", (unsigned long)tama.tokensToday);
    spr.setTextColor(p.textDim, p.bg);
    ln("all     %lu tok", (unsigned long)stats().tokens);
    y += 6;
    spr.setTextColor(p.text, p.bg);
...
static void drawApproval() {
  ...
  spr.setCursor(4, 6);
  if (tama.promptKind == 1) spr.printf("question %u/%u  %lus", tama.promptQi + 1, tama.promptNq, (unsigned long)waited);
  else                      spr.printf("approve?  %lus", (unsigned long)waited);

  // Which repo this came from, right-aligned on the status line.
  if (tama.promptCwd[0]) {
    spr.setTextColor(p.textDim, p.bg);
    spr.setTextDatum(TR_DATUM);
    spr.drawString(tama.promptCwd, W - 4, 6);
    spr.setTextDatum(TL_DATUM);
  }
  ...
  spr.drawFastHLine(0, 27, W, p.textDim);

  // Greedy word-wrap the whole command, then show a scrolling window over it
  // rather than truncating at two lines.
  // A question gives up rows to its option list.
  const uint8_t COLS = 39, MAXROWS = 24;
  const uint8_t VIS = (tama.promptKind == 1 && tama.nOpts > 0) ? 7 - tama.nOpts : 8;
  static char rows[MAXROWS][COLS + 1];
  uint8_t nrows = 0, col = 0;
  const char* s = tama.promptHint;
  rows[0][0] = 0;
  while (*s && nrows < MAXROWS) {
    while (*s == ' ') s++;
    const char* w = s;
    while (*s && *s != ' ') s++;
    size_t wlen = s - w;
    if (!wlen) break;
    while (wlen > 0) {
      if (col >= COLS) { rows[nrows][col] = 0; if (++nrows >= MAXROWS) break; col = 0; rows[nrows][0] = 0; }
      size_t take = wlen < (size_t)(COLS - col) ? wlen : (size_t)(COLS - col);
      if (col > 0 && take == wlen && col + wlen < COLS) rows[nrows][col++] = ' ';
      else if (col > 0 && take < wlen) { rows[nrows][col] = 0; if (++nrows >= MAXROWS) break; col = 0; continue; }
      memcpy(&rows[nrows][col], w, take);
      col += take; w += take; wlen -= take;
      rows[nrows][col] = 0;
    }
  }
  if (col > 0 && nrows < MAXROWS) nrows++;

  if (promptScroll > 0 && promptScroll + VIS > nrows) {
    promptScroll = nrows > VIS ? nrows - VIS : 0;
  }
  spr.setTextColor(p.textDim, p.bg);
  for (uint8_t i = 0; i < VIS && promptScroll + i < nrows; i++) {
    spr.setCursor(4, 32 + i * 10);
    spr.print(rows[promptScroll + i]);
  }
  if (nrows > VIS) {
    spr.setTextColor(p.textDim, p.bg);
    spr.setTextDatum(TR_DATUM);
    char sc[16];
    snprintf(sc, sizeof(sc), "%u/%u", promptScroll + VIS, nrows);
    spr.drawString(sc, W - 4, H - 12);
    spr.setTextDatum(TL_DATUM);
  }

  // Option list under the question, highlight follows askSel.
  if (tama.promptKind == 1 && !denyMode) {
    int oy = 32 + VIS * 10 + 2;
    for (uint8_t i = 0; i < tama.nOpts; i++) {
      bool sel = (i == askSel);
      spr.setTextColor(sel ? GREEN : p.textDim, p.bg);
      spr.setCursor(4, oy + i * 10);
      spr.printf("%c %.37s", sel ? '>' : ' ', tama.promptOpts[i]);
    }
  }

  if (denyMode) {
    spr.fillRect(0, H - 22, W, 22, p.bg);
    spr.drawFastHLine(0, H - 22, W, HOT);
    spr.setTextColor(HOT, p.bg);
    spr.setCursor(4, H - 18);
    spr.print(askText ? "your answer:" : "deny why?");
    spr.setTextColor(p.text, p.bg);
    spr.setCursor(4, H - 8);
    // Keep the caret visible by showing the tail of a long reason.
    size_t rn = strlen(denyReason);
    const char* tail = rn > 36 ? denyReason + (rn - 36) : denyReason;
    spr.printf("%s_", tail);
  } else if (responseSent) {
    spr.setTextColor(p.textDim, p.bg);
    spr.setCursor(4, H - 12);
    spr.print("sent...");
  } else if (tama.promptKind == 1) {
    spr.setTextColor(GREEN, p.bg);
    spr.setCursor(4, H - 12);
    spr.print("Enter pick");
    spr.setTextColor(p.textDim, p.bg);
    spr.setCursor(76, H - 12);
    spr.print("O other  N skip");
  } else {
    spr.setTextColor(GREEN, p.bg);
    spr.setCursor(4, H - 12);
    spr.print("Y ok");
    spr.setTextColor(HOT, p.bg);
    spr.setCursor(44, H - 12);
    spr.print("N no");
    spr.setTextColor(p.textDim, p.bg);
    spr.setCursor(88, H - 12);
    spr.print(nrows > VIS ? "R why  ; . scroll" : "R why");
  }
#else
  const int AREA = 78;
...
void setup() {
  ...
  halImuInit();
  halBeepInit();
  // Claim the sprite before any network task exists: it needs 64,800
  // contiguous bytes, and a failed createSprite is silent — pushSprite just
  // draws nothing, which reads as a dead screen with everything else working.
  spriteReady = spr.createSprite(W, H) != nullptr;
  startBt();
  ...
  statsLoad();
  settingsLoad();
  // Apply the persisted levels; the applyBrightness() above ran before the
  // settings were loaded and only set the compiled-in default.
  brightLevel = settings().bright;
  applyBrightness();
  halVolume(settings().volume);
  petNameLoad();
  buddyInit();
...
void loop() {
  // Drawing into a sprite that never allocated dereferences a null buffer, so
  // hold off rendering and keep retrying instead of faulting.
  if (!spriteReady) {
    ...
    if (!spriteReady) { delay(100); return; }
  }
  halUpdate();
  halBeepUpdate();
  t++;
...
#endif
      } else if (!menuOpen && !settingsOpen && !resetOpen && !onSessionsPage()) {
        sfxNav();
  ...
  if (halBtnB().wasPressed()) {
    ...
    if (inPrompt) {
      ...
      if (!sessActionsOpen) { sfxNav(); infoPage = (infoPage + 1) % INFO_PAGES; }
  ...
  // Drain typed characters while annotating a denial. Quotes and backslashes
  // are dropped so the reason can be embedded in JSON without escaping.
  if (denyMode) {
    char c;
    while ((c = halPollChar()) != 0) {
      if (c == '"' || c == '\\') continue;
      size_t n = strlen(denyReason);
      if (n < sizeof(denyReason) - 1) { denyReason[n] = c; denyReason[n + 1] = 0; }
    }
    wake();
  }
  ...
  while (true) {
    ...
    // Brightness ([ ]) and volume (- =) apply in every mode, including while
    // a prompt is up, so they are handled before anything can consume them.
    // The volume beep doubles as feedback at the new level.
    // Both step the persisted Settings value and go through the stock apply
    // path, so wake()/undim re-apply the same level instead of an older one.
    if (k == HalKey::BrightUp || k == HalKey::BrightDown) {
      if (k == HalKey::BrightUp && brightLevel < 4) brightLevel++;
      if (k == HalKey::BrightDown && brightLevel > 0) brightLevel--;
      applyBrightness();
      settings().bright = brightLevel;
      settingsSave();
      continue;
    }
    if (k == HalKey::VolUp || k == HalKey::VolDown) {
      Settings& s = settings();
      int v = (int)s.volume + (k == HalKey::VolUp ? 10 : -10);
      s.volume = v < 0 ? 0 : v > 100 ? 100 : v;
      s.sound = s.volume > 0;          // a raised volume must be audible
      halVolume(s.volume);
      settingsSave();
      sfxNav();
      continue;
    }
    // s / u jump straight to a window from anywhere except a live prompt,
    // which keeps the approval screen in front until it is answered.
    if ((k == HalKey::Sessions || k == HalKey::Usage) && !(tama.promptId[0] && !responseSent)) {
      menuOpen = false; settingsOpen = false;
      displayMode = DISP_INFO;
      infoPage = (k == HalKey::Sessions) ? INFO_PG_SESSIONS : INFO_PG_USAGE;
      applyDisplayMode();
      sfxNav();
      continue;
    }
    ...
    // Up/Down scroll the wrapped command.
    if (promptLive) {
      // r opens text entry; Enter then denies with what you typed, and Del
      // backspaces, cancelling only once the field is empty.
      if (denyMode) {
        if (k == HalKey::Approve) {
          if (askText) {
            sendAnswer(denyReason);
            sfxApprove();
          } else {
            char cmd[256];
            snprintf(cmd, sizeof(cmd),
                     "{\"cmd\":\"permission\",\"id\":\"%s\",\"decision\":\"deny\",\"reason\":\"%s\"}",
                     tama.promptId, denyReason);
            sendCmd(cmd);
            statsOnDenial();
            sfxDeny();
          }
          responseSent = true;
          denyMode = false; askText = false;
          halSetTextMode(false);
        } else if (k == HalKey::Back) {
          size_t n = strlen(denyReason);
          if (n > 0) denyReason[n - 1] = 0;
          else { denyMode = false; halSetTextMode(false); }
        }
        continue;
      }
      if (k == HalKey::Reason) {
      ...
      // Question prompt: Up/Down move the highlight, Enter/Y pick it,
      // O types a free-text answer, N declines (falls through below).
      if (tama.promptKind == 1 && k != HalKey::Deny) {
        if (k == HalKey::Up && askSel > 0) askSel--;
        else if (k == HalKey::Down && askSel + 1 < tama.nOpts) askSel++;
        else if (k == HalKey::Approve && tama.nOpts > 0) {
          sendAnswer(tama.promptOpts[askSel]);
          responseSent = true;
          sfxApprove();
        } else if (k == HalKey::Other) {
          denyMode = true; askText = true;
          denyReason[0] = 0;
          halSetTextMode(true);
        }
        continue;
    ...
    // Sessions window owns Up/Down/Enter/Back while it is the visible page,
    // so a session can be selected and acted on without opening a modal.
    if (onSessionsPage() && !menuOpen && !settingsOpen && !resetOpen && !petPickerOpen) {
      if (sessActionsOpen) {
        if      (k == HalKey::Up)   { sessActionSel = (sessActionSel + SESS_ACTION_N - 1) % SESS_ACTION_N; sfxNav(); continue; }
        else if (k == HalKey::Down) { sessActionSel = (sessActionSel + 1) % SESS_ACTION_N; sfxNav(); continue; }
        else if (k == HalKey::Approve) {
          sfxConfirm();
          if (sessActionSel < SESS_ACTION_N - 1) sessControl(sessActionSel);
          sessActionsOpen = false;
          consumedEnter();
          continue;
        } else if (k == HalKey::Back) {
          sfxBack(); sessActionsOpen = false; consumedDel(); continue;
        }
      } else {
        if (k == HalKey::Up && tama.nLines) {
          sessSel = (sessSel + tama.nLines - 1) % tama.nLines; sfxNav(); continue;
